(function ($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.erd = {
    attach: function (context, settings) {
      $(once('erd-init', '.erd-container', context)).each(erdInit);
    }
  };
  ...
  function erdInit() {
    ...
    data.graph.on('change', function (cell) {
      storeGraph(cell.graph);
    });
  ...
  function saveClick(event) {
    ...
    image.onload = function () {
      var output_format = drupalSettings.erd.settings.output_format;
  ...
  function createGraph(element) {
    ...
    var graphState = JSON.parse(drupalSettings.erd.graphState);
    if (graphState) {
      graph.fromJSON(graphState);
    }
    else {
      // @todo We still want this set if all items are removed, but right now it prevents the fromJSON render.
  ...
  function getDefaultJointEntity() {
    return new joint.shapes.erd.Entity({
      ...
      attrs: {
        ...
        '.outer': {
          fill: '#1cecff', stroke: '#1cecff', 'stroke-width': 1
        },
        '.port': {
          magnet: 'active',
          r: 7,
   ...
   */
  function createLink(source, target, label, cardinality, graph) {
    ...
    link.addTo(graph).set('labels', [{
      ...
      attrs: {
        text: {
          text: cardinality, fill: '#000000',
          'font-family': 'sans-serif', 'font-size': 14
        },
        rect: { stroke: '#ffffff', 'stroke-width': 5, rx: 1, ry: 1 }
      }
    }]);

    // Link creation doesn't trigger the generic graph change event to update stored state.
    storeGraph(graph);
  }
    ...
    for (var i in drupalSettings.erd.links) {
      ...
      if (from) {
        for (var j in link.targets) {
          ...
          if (to && from !== to) {
            createLink({ id: from.id, selector: link.from_selector }, { id: to.id }, link.label, link.cardinality, graph);
          }
        }
      }
    }
  ...
  function removeClick(event) {
    ...
    storeGraph(event.data.graph);
  }

  /**
   * Saves the graph state to Drupal.
   */
  function storeGraph(graph) {
    const json = graph.toJSON();

    // Remove recursive originalData that cannot be stringified.
    for (var index in json.cells) {
      if (typeof json.cells[index]['erd.bundle'] !== 'undefined') {
        delete json.cells[index]['erd.bundle'].originalData;
      }
      else if (typeof json.cells[index]['erd.type'] !== 'undefined') {
        delete json.cells[index]['erd.type'].originalData;
      }
    }

    $.ajax({
      type: 'POST',
      url: '/admin/structure/erd/ajax',
      dataType: 'json',
